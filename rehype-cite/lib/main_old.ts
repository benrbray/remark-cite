...
// `rehype-cite` reads `extractedBibtex` field from `VFile.data`
...
const processInlineCite = (
  ...
  // TODO: CitationInline is an invalid HTML element name
  // perhaps there should be an `useMdxElementNames` config option
  // so that valid HTML is produced by default, and MDX is only used when needed
...
const runRehypeCite = (config: RehypeCiteConfig) => {
  ...
  const processBibliography = (
    ...
    return {
      ...
      properties: {
        className: ["rehype-cite-bib"],
        citeItemsJson: JSON.stringify(formattedEntries)
      },
